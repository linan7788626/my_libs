...
void sdens_to_kappa(REAL pmass, REAL* sdens, int Ncc, REAL Dcell, REAL zl, REAL zs, REAL *kappa) {
	...
	for (i=0;i<Ncc;i++) for (j=0;j<Ncc;j++) {
		index = i*Ncc+j;
		kappa[index] = pmass*sdens[index]/scrit;
	}
}
//--------------------------------------------------------------------
...
void kappa_to_phi(REAL *kappa, REAL *phi,int Ncc2,REAL Dcell) {
	...
	REAL *green_iso = calloc(Ncc2*Ncc2,sizeof(REAL));
	kernel_green_iso(Ncc2,green_iso,Dcell);
	REAL *phi_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
...
void kappa_to_alphas(REAL *kappa,REAL *alpha1,REAL *alpha2,int Ncc2,REAL Dcell) {
	...
	REAL *alpha1_iso = calloc(Ncc2*Ncc2,sizeof(REAL));
	REAL *alpha2_iso = calloc(Ncc2*Ncc2,sizeof(REAL));

	kernel_alphas_iso(Ncc2,alpha1_iso,alpha2_iso,Dcell);

	REAL *alpha1_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
	REAL *alpha2_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
...
void kappa_to_shears(REAL *kappa,REAL *shear1,REAL *shear2, int Ncc2,REAL Dcell) {
	...
	REAL *shear1_iso = malloc(Ncc2*Ncc2*sizeof(REAL));
	REAL *shear2_iso = malloc(Ncc2*Ncc2*sizeof(REAL));

	kernel_shears_iso(Ncc2,shear1_iso,shear2_iso,Dcell);

	REAL *shear1_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
	REAL *shear2_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
...
void kappa_to_kappac(REAL *kappa,REAL *kappa_cal, int Ncc2,REAL Dcell) {
	...
	REAL *kappa1_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
	REAL *kappa2_tmp = malloc(Ncc2*Ncc2*sizeof(REAL));
...
void sdens_to_phi(REAL pmass, REAL * sdens, int Nc, REAL boxsize, REAL zl, REAL zs, REAL * phi) {
	...
	REAL *kappa0 = calloc(Nc*Nc,sizeof(REAL));
	sdens_to_kappa(pmass, sdens, Nc, dsx, zl, zs, kappa0);
